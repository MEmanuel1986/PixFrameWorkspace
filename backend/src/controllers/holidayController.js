 ...
 *
 * Alle Ergebnisse werden 24h im Datei-Cache gespeichert.
 */
...
const CACHE_DIR = path.join(paths.DATA_DIR, 'holiday_cache');
...
function slugToFerienName(slug) {
  ...
  // Strip year suffix e.g. "winterferien-2026-mecklenburg-vorpommern"
...
function cacheFile(type,state,year) {
  return path.join(CACHE_DIR,`${type}_${state.toUpperCase()}_${year}.json`);
}
...
function writeCache(file,data) {
  try { fs.writeFileSync(file,JSON.stringify(data),'utf8'); } catch(e) {
    console.warn('[holidays] Cache write failed:',e.message);
  }
}
...
async function loadFeiertageForStateYear(state, year) {
  ensureCacheDir();
  const file=cacheFile('feiertag',state,year);
  const cached=readCache(file);
  if (cached) return {data:cached,source:'cache'};

  // Quelle 1: feiertage-api.de
  try {
    const url=`https://feiertage-api.de/api/?jahr=${year}&nur_land=${state.toUpperCase()}`;
  ...
  // Fallback: Gauß-Offline
  const data=computeFeiertageOffline(year,[state]);
  // Fallback nicht persistieren, damit nächste Anfrage API erneut versucht
...
async function loadFerienForStateYear(state, year) {
  ensureCacheDir();
  const file=cacheFile('ferien',state,year);
  const cached=readCache(file);
  if (cached) return {data:cached,source:'cache'};

  // Quelle 1: openholidaysapi.org — menschlesbare Namen auf DE
  try {
    ...
    // Response: [{ "startDate":"2026-02-23","endDate":"2026-02-28","name":[{"language":"DE","text":"Winterferien"}] }]
    if (Array.isArray(json)&&json.length>0) {
      const data=json.map(h=>{
        const nameObj=(h.name||[]).find(n=>n.language==='DE')||(h.name||[])[0]||{};
        return {
          name:  nameObj.text||'Schulferien',
          start: h.startDate||null,
          end:   h.endDate  ||null,
        };
  ...
  // Quelle 2: ferien-api.de (Backup, Slug-Namen werden gemappt)
  try {
    const url=`https://ferien-api.de/api/v1/holidays/${state.toUpperCase()}/${year}`;
    const json=await fetchJSON(url,8000);
    // Response: [{ "start":"2026-02-23T00:00","end":"2026-02-28T00:00","name":"winterferien","stateCode":"MV" }]
    // end ist INKLUSIVE letzter Ferientag
    if (Array.isArray(json)&&json.length>0) {
      const data=json.map(h=>({
  ...
  // Kein Fallback für Schulferien (keine zuverlässige Offline-Berechnung möglich)
...
class HolidayController {
  ...
  clearCache(req,res) {
    try {
      ...
      res.json({deleted:files.length,message:`${files.length} Cache-Dateien gelöscht`});
    } catch(e){
