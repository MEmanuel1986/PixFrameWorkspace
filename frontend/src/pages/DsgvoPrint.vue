<template>
  ...
  <template v-else>
    ...
    <!-- Fixer Seitenfuß für Druck -->
    <footer class="pf no-screen">
      <hr class="pf-rule" />
      <div class="pf-top">
        <span class="pf-left">Einwilligungserklärung gem. DSGVO · {{ settings.company.name }}</span>
    ...
    <div class="page-wrap">
      <div class="a4">
        ...
        <!-- Briefkopf: Absenderzeile + Logo wie im Muster -->
        <div class="top-bar">
        ...
        <div class="doc-title-block">
          <h1 class="doc-title">Einwilligungserklärung gem. DSGVO</h1>
          <div class="doc-sub">Art. 6 Abs. 1 lit. a, Art. 7 DSGVO · § 22 KUG · {{ settings.company.name }} · {{ fmtDate(new Date().toISOString()) }}</div>
        </div>

        <!-- Alle Abschnitte aus den Settings -->
        <div v-for="(para, pIdx) in allParagraphs" :key="para.id" class="section">
          <div class="sh-row">
            <div class="sh-pill">{{ pIdx + 1 }}</div>
            <h2 class="sh">{{ para.title }}</h2>
          </div>

          <!-- Verantwortlicher: Info-Box mit Firmendaten -->
          <template v-if="para._isVerantwortlicher">
            <div class="infobox-contact">
              <strong>{{ settings.company.name }}</strong><br />
              <span v-if="settings.company.owner" >Inh. {{ settings.company.owner }}<br /></span>
              <span >{{ settings.company.street }}, {{ settings.company.zipCode }} {{ settings.company.city }}<br /></span>
              <span  v-if="settings.company.phone || settings.company.email">
                {{ [settings.company.phone, settings.company.email].filter(Boolean).join(' · ') }}
              </span>
            </div>
            <ul class="ul-list mt-3" v-if="para.items && para.items.length > 1">
              <li v-for="(item, iIdx) in para.items.slice(1)" :key="iIdx">{{ item }}</li>
            </ul>
          </template>

          <!-- Einwilligung §22 KUG: Checkboxen -->
          <template v-else-if="para._isEinwilligung">
            <div class="cb-list mt-3">
              <div class="cb-row">
                <span class="cb">☐</span>
                <span><strong>Einwilligung erteilt:</strong> Der Auftragnehmer darf die Aufnahmen für eigene Werbezwecke nutzen (Portfolio, Website, Social Media, Printmedien, Wettbewerbe).</span>
              </div>
              <div class="cb-row">
                <span class="cb">☐</span>
                <span><strong>Einwilligung unter Vorbehalt:</strong> Nutzung nur nach schriftlicher Absprache im Einzelfall.</span>
              </div>
              <div class="cb-row">
                <span class="cb">☐</span>
                <span><strong>Keine Einwilligung:</strong> Die Aufnahmen dürfen nicht für eigene Werbezwecke genutzt werden.</span>
              </div>
            </div>
            <p class="p mt-3">Diese Einwilligung ist freiwillig und kann jederzeit mit Wirkung für die Zukunft widerrufen werden (Art. 7 Abs. 3 DSGVO).</p>
          </template>

          <!-- Alle anderen: Aufzählung -->
          <template v-else>
            <ul class="ul-list mt-2">
              <li v-for="(item, iIdx) in (para.items || [])" :key="iIdx">{{ item }}</li>
            </ul>
          </template>
        </div>

        <!-- Unterschriften -->
        <div class="sig-grid mt-6">
          <div class="sig-block">
            <div class="sig-line"></div>
            <div class="sig-label">Ort, Datum und Unterschrift Auftragnehmer</div>
          </div>
          <div class="sig-block">
            <div class="sig-line"></div>
            <div class="sig-label">Ort, Datum und Unterschrift Auftraggeber</div>
          </div>
        </div>
        ...
        <footer class="screen-footer">
            ...
            <div>
              <span v-if="settings.company.phone">{{ settings.company.phone }}<br /></span>
  ...
  setup() {
    ...
    async function fetchAll() {
      try {
        ...
        const raw = Array.isArray(s.dsgvoParagraphs) ? s.dsgvoParagraphs : []
        dsgvoParagraphs.value = raw.map(p => ({
          ...p,
          _isVerantwortlicher: p.title?.toLowerCase().includes('verantwortlich'),
          _isEinwilligung: p.title?.toLowerCase().includes('einwilligung') || p.title?.toLowerCase().includes('veröffentlichung'),
        }))
      } catch (e) {
...
.infobox-contact strong { color: #1e3a8a; }
