<template>
  <div v-if="loading" class="state-screen"><div class="state-icon">⏳</div><div>AGB werden geladen…</div></div>
  ...
  <template v-else>
    ...
    <footer class="pf no-screen">
      <hr class="pf-rule" />
      <div class="pf-top">
        <span class="pf-left">Allgemeine Geschäftsbedingungen · {{ settings.company.name }}</span>
      ...
      <div class="pf-cols">
        ...
        <div class="pf-col">
          ...
          <span v-if="settings.company.vatId && !settings.company.smallBusiness">USt-IdNr.: {{ settings.company.vatId }}</span>
    ...
    <!-- ── A4 Seite ── -->
    <div class="page-wrap">
      <div class="a4" id="agb-print-area">
        ...
        <div class="doc-title-block">
          <h1 class="doc-title">Allgemeine Geschäftsbedingungen</h1>
          <div class="doc-sub">{{ settings.company.businessType }} · Stand: {{ fmtDate(new Date().toISOString()) }}</div>
        </div>

        <!-- AGB Paragraphen -->
        <div class="agb-body">
          <div v-for="(para, pIdx) in agbParagraphs" :key="para.id" class="agb-para">
            <div class="sh-row">
          ...
          <div v-if="agbParagraphs.length === 0" class="agb-empty">
            Noch keine AGB-Paragraphen vorhanden. Bitte in Einstellungen → AGB pflegen.
          </div>
        </div>

        <!-- Schlusszeile -->
        <div class="agb-footer-note">
          <hr class="closing-rule" />
          <p>
            Diese Allgemeinen Geschäftsbedingungen gelten für alle Aufträge und Leistungen von
          ...
          <p class="agb-place-date">{{ settings.company.city }}, {{ fmtDate(new Date().toISOString()) }}</p>
        </div>
        ...
        <footer class="screen-footer">
            ...
            <div class="footer-col">
              ...
              <div v-if="settings.company.smallBusiness"><em>Kleinunternehmer § 19 UStG</em></div>
            </div>
          </div>
        </footer>
  ...
  setup() {
    ...
    async function fetchAll() {
      try {
        ...
        agbParagraphs.value = Array.isArray(s.agbParagraphs) ? s.agbParagraphs : []
      } catch (e) {
...
.para-item {
  font-size: 8.5pt; line-height: 1.65; color: #1a1a1a;
  padding: 1.5px 0 1.5px 7mm; position: relative;
  orphans: 3; widows: 3; margin-bottom: 1mm;
}
.agb-para  { counter-reset: para-item; }
.para-item { counter-increment: para-item; }
.para-item::before { content: counter(para-item) "."; position: absolute; left: 0; color: #888; font-size: 8pt; }
.agb-footer-note { margin-top: 4mm; break-inside: avoid; }
.agb-footer-note p { font-size: 8pt; color: #555; line-height: 1.6; }
.agb-place-date   { font-size: 8pt; color: #777; margin-top: 2mm; }
